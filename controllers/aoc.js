...
function in_aoc_geom_com(geometry) {
    return format('select st_asgeojson(ST_TRANSFORM(appellation.geom, 4326)) as geom, \
...
function bbox_aoc(bbox) {
    return format('select st_asgeojson(st_transform(Appellation.geom, 4326)) as geom, id_uni, appellation, commune \
